...
/*
	event listener idea
		-> assign [event name, object to callback, callback function]
*/
...
with (EVENT_LISTENER) {
	...
	event_assign = function(_event_name, _object, _callback) {
		var _new_listener = {
			object: _object,
			callback: _callback
		};
	...
	event_unassign = function(_event_name, _object, _callback) {
		...
		if (ds_map_exists(EVENT_LISTENER.list, _event_name)) {
			...
			for (var i = array_length(_listeners_array) - 1; i >= 0; i--) {
				...
				if (_listener.object == _object && _listener.callback == _callback) {
	...
	event_overwrite = function(_event_name, _object, _callback, _new_callback) {
		event_unassign(_event_name, _object, _callback);
		event_assign(_event_name, _object, _new_callback);
